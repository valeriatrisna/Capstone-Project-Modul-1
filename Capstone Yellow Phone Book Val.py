...
def create_data(data,user):
    while True:
        ...
        while True:
            validasi = input( f'Apakah anda yakin menambahkan data Nama : {namaBaru}, Code : {CodeBaru}, Nomor : {NomorBaru}, email : {EmailBaru}, Jenis Kelamin : {JKBaru}, Domisili : {DomisiliBaru} Y/N? ').title()
            if validasi == 'Y':
        ...
        while True:
            opsi = input('Apakah Anda ingin menambahkan data lain? (Y/N)').capitalize()
            if opsi == 'Y':
                break
            elif opsi == 'N':
                print('Terima kasih')
...
#function untuk mengupdate data dengan input (data & user), update data berdasarkan Code Unik dari tabel yang ditampilkan
    ...
    while True:
        ...
        if re.fullmatch(r'\b\w{5}',ganti):
            ...
            if ganti in lst:
                ...
                while True:
                    validasi = input(f'Apakah anda yakin mengganti data Code : {ganti} menjadi Nama : {NamaGanti}, Code : {CodeGanti}, Nomor : {NomorGanti}, email : {EmailGanti}, Jenis Kelamin : {JKGanti}, Domisili : {DomisiliGanti} Y/N ? ').upper()
                    if validasi == 'Y':
                        for dict1 in data:
                            if ganti == dict1['Code']:
            ...
            while True:
                opsi = input('Apakah Anda ingin mengganti data lain? (Y/N)').capitalize()
                if opsi == 'Y':
                    break
                elif opsi == 'N':
                    print('Terima kasih')
...
def delete_data(data,user):
    read_data(data)
    while True:
        list_code = [i['Code'] for i in data]
        code = input('Masukkan Code data yang ingin Anda hapus: ').upper()
        if code in list_code:
            for dict in data:
                if dict['Code'] == code:
                    confirmation2 = input(f'Apakah Anda yakin menghapus data {dict}? (Y/N)').capitalize()
                    if confirmation2 == 'Y':
                        data.remove(dict)
                        deleted_data.append(dict)
                        read_data(data)
            ...
            print('Code yang Anda masukkan tidak terdaftar!')
        while True:
            opsi = input('Apakah Anda ingin menghapus data lain? (Y/N)').capitalize()
            if opsi == 'Y':
                break
            elif opsi == 'N':
                print('Terima kasih')
...
def menulogin(data):
    print('''
          Silahkan masukkan User & Password Anda
          ''')
